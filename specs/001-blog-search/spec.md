...
4. **Given** a user enters an empty search query, **When** they submit the search, **Then** no results are shown (empty state message displayed)
...
4. **Given** multiple search results are displayed, **When** a user views the results, **Then** results are ordered by date (newest first)
...
- Should search be case-sensitive or case-insensitive? **RESOLVED**: Case-insensitive (search matches "Next" and "next" as the same)
...
- **FR-010**: System MUST display no results when an empty search query is submitted
...
- Search matching will be case-insensitive (confirmed)
- Search will support partial word matching (confirmed)
- Search results will be ordered by date, newest first (confirmed)
- Empty search queries will show no results (confirmed)
